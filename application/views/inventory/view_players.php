		...
		<tr id="player_<?=$this_player['id']?>">
			<td width="40">
				<?php if (file_exists(APPPATH . '../assets/images/profile_pictures/' . $this_player['id'] . '_thumb.jpg')): ?>
					<img width="40" height="40" src="<?='assets/images/profile_pictures/' . $this_player['id'] . '_thumb.jpg'?>" alt="<?=$this_player['name']?>">
				<?php else: ?>
					<img width="40" height="40" src="<?='assets/images/profile_pictures/nopic_thumb.jpg'?>" alt="<?=$this_player['name']?>">
				<?php endif; ?>
			</td>
			...
			<td><span<?php echo(($this_player['last_activity_unix'] > (time() - 500)) ? ' style="color: #0FB308"' : '')?>><?=substr($this_player['last_activity'], 0, -3)?></span></td>
		</tr>
	<?php endforeach; ?>
</table>
