...
class Chat extends Main
{
    ...
    public function index()
    {
        if ($this->data['user']['verified'] == 1) {
            $this->load->view_ajax('chat/view_chat.php', $this->data);
        }
    }
    ...
    public function update_chat()
    {
        ...
        $sql = "UPDATE " . $this->db->game_table . " SET last_activity = now() WHERE user_id = '" . $this->data['user']['id'] . "'";
        $this->db->query($sql);
        ...
        $this->data['chat'] = $chat_content->result_array();
        
        $player_input['online_only'] = true;
        $this->data['online_users'] = $this->User->get_players($player_input);

        $this->load->view_ajax('chat/view_content', $this->data);
    }
    ...
    public function post_chat()
    {
        if ($this->data['user']['verified'] == 1) {
            ...
            if (! empty($chat_post['entry'])) {
                $chat_post['user_id'] = $this->data['user']['id'];
                $chat_post['name'] = $this->data['user']['name'];
                $chat_post['time'] = date('Y-m-d H:i:s', time());
                $chat_post['place'] = ucwords($this->data['game']['town']) . 's ' . $this->data['game']['place'];
