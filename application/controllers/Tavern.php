	...
	function sailors()
	{
		if ($this->user['game']['event_sailors'] != 'banned')
		{
			...
			elseif ($action > 50 && $action <= 85)
			{
				...
				if ($this->user['user']['sound_effects_play'] == 1)	{ $data['playSound'] = 'sword_fight';	}
	...
	function gamble_post()
	{
		...
		else
		{
			...
			else
			{
				...
				if ($this->user['user']['sound_effects_play'] == 1)	{ $data['playSound'] = 'dices';	}
			}
