...
class GAMELIB {
	...
	function ship_spec($manned_cannons, $nation)
	{
	...
	function get_crew_friendly_mood($crew_mood)
	{
		switch ($crew_mood) {
			case ($crew_mood <= -10):
				return "aggressive";
	...
	function get_crew_health_symbol($crew_health)
	{
		switch ($crew_health)
		{
			case ($crew_health <= 25):
	...
	function report_crew_unhappiness($crew)
	{
		$angry_crew = 0;
	...
	function get_inventory_ship($number_of_ships, $ship_health)
	{
		if ($ship_health <= 25) { $health_symbol = 'ship_health_25'; }
		else if ($ship_health > 25 && $ship_health <= 50) { $health_symbol = 'ship_health_50'; }
		else if ($ship_health > 50 && $ship_health <= 75) { $health_symbol = 'ship_health_75'; }
		else { $health_symbol = 'ship_health_100'; }
	...
	function random_greeting($place, $name, $gender, $age)
	{
	...
	function validate_form($input, $rules)
	{
